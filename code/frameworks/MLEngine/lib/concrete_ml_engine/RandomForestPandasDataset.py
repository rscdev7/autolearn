...
Questo componente implementa un wrapper ad un'implementazione dell'algoritmo RandomForest
"""
...
class RandomForestPandasDataset (Model):

    __slots__ = ("_modelImpl" , "_computeDevice" , "_max_depth" , "_n_estimators")
    def setUp (self, pHyperParams:dict = {} , pComputeDevice:str="cpu") -> Union[ None , Exception ]:
        ...
        """
        try:
            if "max_depth" in pHyperParams.keys() and "n_estimators" in pHyperParams.keys():
                self._max_depth:str                         = pHyperParams['max_depth']
                self._n_estimators:int                      = pHyperParams['n_estimators']
                self._modelImpl:RandomForestClassifier      = RandomForestClassifier(max_depth=self._max_depth, n_estimators=self._n_estimators)

            elif "max_depth" in pHyperParams.keys():
                self._max_depth:str                         = pHyperParams['max_depth']
                self._modelImpl:RandomForestClassifier      = RandomForestClassifier(max_depth=self._max_depth)

            elif "n_estimators" in pHyperParams.keys():
                self._n_estimators:int                      = pHyperParams['n_estimators']
                self._modelImpl:RandomForestClassifier      = RandomForestClassifier(n_estimators=self._n_estimators)

            else:
                self._modelImpl:RandomForestClassifier      = RandomForestClassifier( )
